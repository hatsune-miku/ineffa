...
状态：0.1.0 实现基线。已完成 Bun + OpenCode 2.0.3 进程内接入、WebUI 与自动化集成验证；KOOK 真实网关的长时间稳定性仍需部署环境验证。运行方式见 [README.md](README.md)。
...
默认采用**一个 Bun 进程，进程内嵌入 OpenCode**。官方 v2 的 [`@opencode/sdk`](https://opencode.ai/v2/docs/build/sdk) 提供 `OpenCode.create()` 和显式关闭能力，无须额外启动 HTTP 服务。实现固定使用 `2.0.3`，并显式传入数据库路径；SDK 默认的内存数据库不用于正式服务。
...
`/debug on|off` 将会话级开关保存在绑定记录中，显式重置继承该开关。计时使用 OpenCode 官方 HTTP / tool 钩子；钩子统一注册，按会话开关决定是否采样；开关不再更换 SDK instance，因此不会重建浏览器等有状态的 MCP 连接，用户提供的 instance 配置仍保留。在模型 SSE 响应流上透传字节，收到第一个完整数据事件时记录首字（相对本轮首次请求），不等待正文、不统计心跳注释，不复制或积存响应流，取消与流错误继续向上下游传播。连接/响应头汇总最小、最大、平均值；总计和工具累计耗时按时分秒显示。请求和工具期间不额外调用模型。进程内采样在本轮结束时清理，完成报告单独落盘；平台投递与 WebUI 展示时附加，OpenCode 原始消息和 Agent 转交文本均保持不变。进程退出会丢失尚未完成的采样，报告标明采集边界，不宣称能恢复未观测到的连接或首字耗时。

Debug 的「服务排队」是粗略估算，直接复用首字耗时：本轮首次模型 HTTP 请求发出至收到远端第一个完整 SSE 数据事件，按 `26h 11m 5s` 格式显示；不足一秒为 `0s`，未收到事件为「未采集」。不累加工具后的后续请求；若首次请求失败后重试，仍从本轮首次请求起算，包含重试间隔。

这个估算假设缓存命中较高、低负载时首个事件通常能在约两秒内返回，因此忽略网络、请求处理、prefill 和事件生成的时间。不按缓存命中率乘除，也不强制要求达到 95% 才显示。供应商可能在真正排队完成前就发送角色等 SSE 数据事件，也可能缓冲事件后再发送，因此结果可能偏小、偏大，或与正文出现时间明显不符；不针对供应商修正或过滤。这不是供应商上报的队列时长，也不能区分限流、资源调度和真实排队。说明保留在文档中，不追加到每次 debug 输出。
...
- 一轮回答共用一张回复卡片，正文使用 KMarkdown；输出 Token、工具次数、Thinking 状态和最终 Debug 报告放在分割线下的备注模块，不再分别发送状态消息。工具调用前后的正文按 step 顺序合并；同次 execution 中随后处理的排队输入创建新的回复卡片，原生 OpenCode 历史不变。500 ms 合并窗口与消息最小更新间隔仅作用于平台投递，不阻塞模型。Token 累计本轮各 step 的 output + reasoning，不包含输入和缓存；没有用量时显示 `—`。超长正文最终上传完整 Markdown 文件，统计仍保留在原卡片中。
- 草稿与最终回复以本轮首个 assistant message ID 复用同一条 outbound、平台消息 ID；正文、备注和 revision 一起持久化。重启从原生日志恢复工具计数和已完成 step 的正文。只有最终正文的最新版本确认送达后才转交 mention，备注不参与转交或模型上下文。创建结果未知不盲目重发，旧回声不能确认新版本编辑。
...
所有平台的系统提示固定加入：`你由 OpenCode 驱动，但不代表你是编程/开发特化的 Agent——不要假设用户的专业领域，你只作为通用智能处理用户的请求。`
...
浏览器使用锁定版本的 Playwright MCP，通过 OpenCode 原生 stdio MCP 生命周期运行无头 Chromium，按工作目录共享临时浏览器配置。嵌入时在原生 MCP 首次工具快照之前注册默认配置；不暴露依赖桌面连接的 OpenCode browser RPC 工具。Debug 通过每个事件所属会话判断是否采集，不切换运行实例，避免断开有状态的 MCP 连接。安装与隔离边界见 [浏览器说明](docs/browser.md)。
...
当前 KOOK SDK 的 PONG 校验补丁、媒体上传协议和 question 交互边界见 [KOOK 接入说明](docs/kook.md)。附件下载与平台上传属于 Adapter；Core 仅扩展附件投递记录和明确的 `send_file` 工具。交互记录只维护原生表单的回答者、逐题答案及平台去重，不接管 OpenCode 的工具执行。原生表单在进程重启后失效，必须可见地关闭旧问题，不能自行重跑模型来恢复。
...
- WebUI：会话创建 / 搜索 / 归档 / 重置 / 改名 / 模型与 Agent 切换、原生上下文压缩与 JSON 导出、实时文本、排队输入操作、权限规则与表单回复、模型 Key 连接、KOOK 账号管理和投递核实。运行设置提供 MCP 管理、Skills 查询、Agent 配置、默认模型与自动压缩设置。[操作与生效边界](docs/webui.md)
- 配置入口直接修改 OpenCode 原生 JSON/JSONC 的指定字段，保留其他字段和注释；与模型配置和导入共用写入锁。MCP 通过原生接口同步当前工作目录实例，Agent 和运行设置提示重启生效；不引入独立工具注册表或配置数据库。
- 自动化测试实际驱动 OpenCode 的模型请求与工具执行，检查 A/B/C 汇报顺序、后续上下文、重复输入、恢复与发送确认。测试模型服务在本机，平台发送边界使用可控 Adapter。
- 待真实部署验收：KOOK 网关与回声、平台限流 / 网络故障、长时间稳定性。架构和单次测试不等于已经证明长期无故障。
